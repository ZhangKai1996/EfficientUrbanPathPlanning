...
class DQNTrainer:
    ...
    def update_q_network(self, t, update_rate=100):
        if t < int(1e4) or t % 10 != 0: return None, None

        # Sample a batch of experiences from the buffer
        batch = self.replay_buffer.sample_(self.batch_size, num_keys=64)
...
def train_dqn(graph, p,
    ...
    for episode in range(1, num_episodes + 1):
        ...
        while not done:
            ...
            # Update Q network
            loss, entropy = trainer.update_q_network(t=step)
            if loss is not None:
                loss_stats.append([loss, entropy])
        ...
        if episode % save_rate == 0:
            ...
            if len(loss_stats) > 0:
                ...
                writer.add_scalar('Loss', mean_loss[0], episode)
